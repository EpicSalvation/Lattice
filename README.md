...
│   ├── procedural-sky/                    # M17: gradient sky (day/dusk/space + day-night cycle)
...
# Run HUD and controls (M17): a first-person walk/mine/build on the hardness
...
- [x] Discussion task: perform a sanity check. Considering the current feature set, design philosophy, stated goals, and typical game engine behaviors and features, are there any other features, tools, or knobs that we should consider for the initial release of the engine? Add tasks to this milestone (or an additional milestones prior to release, if appropriate) to address accepted features and knobs.
  ...
  - [x] **Discussion + task injection:** reviewed `docs/m17-release-sanity-check.md`; dispositions below. **Accepted** (new M17 tasks injected): A1+A2 lighting/AO, A5 translucent-sort verify, A7 frustum culling (folded into profiling), D1 leveled logging, D2 engine metrics surface, D5 plugin ABI-version guard, E1 `.qb` import/export implementation, B1 reference kinematic-body plugin suite, C1 reference input plugins (keyboard/mouse + gamepad), C2 demo with meaningful HUD, B3 save-versioning contract, D3 configuration guide + runtime tunables. **Deferred** (post-1.0, recorded in `m17-release-sanity-check.md`): A3 time-of-day, A4 particles, A6 texture animation, A8 greedy meshing, A9 post-processing, D4 hot-reload, E2 scripting, E3 unified asset manager, F1 auth/encryption, F2 dedicated-server, G1 asset bundling, G2 additional backends. **Non-goal:** B2 rigid-body physics, B5 weather/wind. **Already tracked:** B4 memory backstop (profiling pass), C3 surface-normal camera
- [x] **Voxel lighting model (sanity-check A1):** implement propagated sky light + block (emitter) light as an engine-owned sparse field overlay — the same shape as fluid/thermal overlays (§17). The renderer samples light levels per vertex/face; emitter values come from materials/plugins. This is the single most recognizable missing voxel-engine feature and the headline visual gap — *shipped: `LightingSystem` (`src/simulation/LightingSystem.h/.cpp`) owns a `FieldOverlay` of combined brightness (ambient default `tuning::lighting::kAmbientBrightness`). Sky light floods downward from unobstructed columns; block light propagates via BFS from materials with `light_emission > 0` and from plugin-registered point sources (`register_light_source`), attenuating by `kAttenuationPerStep` per voxel hop. The mesher (`buildChunkMeshData`) accepts an optional `LightQueryFn` callback and multiplies per-face vertex color by the sampled light level; when null the pre-M17 fixed directional shading is byte-identical. `MaterialProperties` gains `light_emission`; `plugin_api.h` adds `LightingEvent`, `register_on_lighting_event`, and `register_light_source`. Persistence (`.vxc` v2) and network packets serialize the new field. Tuning knobs in `tuning::lighting` (ambient floor, attenuation step, per-frame budgets). Tests in `tests/LightingSystemTest.cpp`*
- [x] **Ambient occlusion in the mesher (sanity-check A2):** per-vertex AO darkening at concave voxel corners ("smooth lighting"). Cheap, large visual return; pairs with the lighting model (A1). Standard technique: for each vertex, count opaque neighbors in the 2×2 column and interpolate — no new data structures needed, just a mesher-local computation — *shipped: `buildChunkMeshData` computes a per-vertex AO level (0–3) from the opaque voxels in the 2×2 block around each corner in the air layer just outside the face, and folds the level's brightness factor (`tuning::ao::kVertexFactor`) into the vertex color alongside the directional shade and optional light. Includes the standard quad-flip (split along the diagonal that isolates the darker corner) so AO does not smear across a face from interpolation anisotropy. Purely mesher-local: out-of-chunk and translucent neighbors are treated as non-occluding, so a concave corner straddling a chunk seam is faintly under-darkened there. Open/flat terrain is level 3 (factor 1.0) → unchanged. Tests in `tests/ChunkMeshDataTest.cpp`*
- [x] **Verify translucent draw ordering (sanity-check A5):** translucent water exists (M14 `flow`); verify the draw ordering is correct for overlapping translucent voxels. If incorrect, fix as a renderer bug — *fixed: chunks were submitted in arbitrary order; translucent faces alpha-blended incorrectly when a nearer chunk was drawn before a farther one. Added back-to-front sort of `pendingChunks` by squared camera distance before the submission loop in `BgfxRenderer::renderFrame`. Architecture doc updated*
- [x] **View-frustum culling (sanity-check A7):** submit only chunks intersecting the camera frustum; today all chunks within view distance are submitted. Fold into the profiling pass — measure first, implement if chunk submission cost is material — *shipped: `Frustum` struct (`include/renderer/Frustum.h`) provides a conservative bounding-sphere test against the five frustum planes (near, far, left, right, top, bottom). `BgfxRenderer::render()` builds the frustum from the current camera state each frame and culls `pendingChunks` via `std::remove_if` before sorting and GPU submission. `renderChunk()` accepts an optional `chunkSizeVoxels` parameter (default 32) to compute each chunk's bounding sphere. The three demos that previously duplicated a local `Frustum` struct (10, 16, 17) now use the engine-level culling automatically; all other demos gain frustum culling with no code changes*
- [x] **Leveled / structured logging (sanity-check D1):** extend `Log` (currently `warn()`-only) to support info/debug/error levels and category tags. Cheap, high developer-experience return
- [x] **Engine metrics surface (sanity-check D2):** expose frame time, draw calls, resident-chunk counts, decomposition queue depth, voice count as a queryable engine-side struct — replacing the ad-hoc per-demo HUD stat recomputation. Complements the profiling pass; also provides data for any future HUD/UI seam
- [x] **Plugin ABI-version guard (sanity-check D5):** stamp native plugins with an ABI version at build time; the engine checks the stamp at load and rejects mismatched plugins with a clear diagnostic rather than silent corruption/crash. Small, protects every consumer and the AI-agent-friendliness goal
- [x] **Implement `.qb` (Qubicle) import/export (sanity-check E1):** the README claims "Full import/export" for `.qb` but no handler exists — only `.vox` is implemented. Implement a `QbImporter` and `QbExporter` following the same pattern as the existing `VoxImporter`/`VoxExporter`. The `.qb` format is simpler than `.vox` (flat voxel grid, no scene graph), so this should be a smaller lift (~400–500 lines). Register in the format registry so it's discoverable alongside `.vox` — *shipped: `QbImporter` (`src/io/QbImporter.h/.cpp`) parses both uncompressed and RLE-compressed .qb files with RGBA/BGRA color format support and visibility-mask handling; unique RGBA colors are mapped to palette indices 1..255 and installed into the engine's visual palette (warns if >255 unique colors). `QbExporter` (`src/io/QbExporter.h/.cpp`) serializes a layer region to uncompressed .qb, converting each voxel's palette_index to its RGBA color via `palette::color`. `Engine::importQb`/`Engine::exportQb` facade methods mirror the `.vox` pattern (plugin-priority dispatch, lossy-property warning). Built-in handlers registered in `PluginManager::registerBuiltinHandlers()` alongside `.vox`. Tests in `tests/QbImportExportTest.cpp`: round-trip color preservation, non-zero anchor placement, two-matrix positioning, RLE decompression, BGRA color format decoding, parser header/truncation/empty-matrix validation, lossy-property warning/no-warning, format registry presence*
- [x] **Reference kinematic-body plugin suite (sanity-check B1):** ship a **reference plugin** (or small plugin suite) that provides a reusable multi-body kinematic system — body registry, gravity integration, ground detection, sweep-and-resolve stepping for N AABB bodies — built on the engine's existing `VoxelCollision::moveAABB` primitive. The engine core adds only the **per-frame body-tick hook** needed to drive the plugin (if one doesn't already exist); all policy lives in the plugin. Ships with the engine so games get multi-body kinematics out of the box, but since it's a plugin, developers can swap, extend, or ignore it. Also serves as a teaching example for M18's template series. This replaces the earlier "document as game-owned" disposition — the friction of every game re-deriving the same gravity+stepping loop is too high — *shipped: engine core adds two new plugin ABI hooks — `register_on_tick(ctx, fn, user_data)` for per-frame callbacks and `move_aabb(ctx, ...)` exposing `VoxelCollision::moveAABB` as a flat-POD collision primitive (ABI version bumped to 2). Both are owner-tracked and torn down on plugin unload via `eraseOwned`. The kinematic-body plugin (`plugins/kinematic-body/plugin.cpp`, shared API header `kinematic_body.h`) provides: `kinbody::API` function-pointer table (create_body, destroy_body, set_input, get_state, set_gravity, set_position, body_count), `BodyDesc` with sensible defaults (0.6×1.8×0.6 AABB, 25 m/s² gravity, 8 m/s walk, 9 m/s jump), `BodyInput` (wish direction + jump), `BodyState` (center, velocity, grounded, per-axis hit flags). The tick function integrates gravity along an arbitrary direction, applies jump opposite to gravity when grounded, combines wish-direction walk with velocity, sweeps via `move_aabb`, and zeros blocked velocity components. Tests in `tests/KinematicBodyTest.cpp`: 15 tests covering tick-hook registration/unload, move_aabb through the plugin ABI (floor landing, no-world fallback), body create/destroy, get_state, gravity fall-and-land, free-fall, jump from ground, jump-in-air rejection, wish-direction horizontal movement, multi-body independent stepping, runtime set_gravity, and teleport via set_position*
- [x] **Reference input plugins (sanity-check C1):** ship two **reference plugins** — one for keyboard/mouse input mapping and one for basic gamepad/controller input — that provide the common boilerplate (action mapping, rebindable keys, dead-zone handling) on top of raw GLFW polling. Not a full input framework, but enough that an indie dev (or AI agent) can get moving without hand-coding hardware input signals. Like the kinematic-body plugin, developers can use as-is, extend, or replace — *shipped: two plugins following the kinematic-body shared-header + API-table pattern, needing **no new engine seam** (input is read host-side before the game step, so the host drives them directly rather than through `register_on_tick`). Crucially the engine owns the window — and therefore GLFW — so each plugin takes a tiny host-supplied **`RawSource`** poll adapter (a few one-line wrappers over `glfwGetKey`/`glfwGetMouseButton`/`glfwGetCursorPos`, and `glfwGetGamepadState`) and pulls raw state through it; **no GLFW type crosses the plugin boundary** — key/button/axis indices are plain ints (GLFW's stable `GLFW_KEY_*`/`GLFW_GAMEPAD_*` constants) and the gamepad snapshot is a flat POD mirroring `GLFWgamepadstate`, so both plugins stay GLFW-free and runtime-loadable. **`plugins/keyboard-mouse`** (`kbinput::api()`, shared `keyboard_mouse.h`) provides named, **rebindable** button actions bound to keys or mouse buttons, two-key **axes** (e.g. W/S → forward, −1..+1), per-frame **edge detection** (`held`/`pressed`/`released`), and a sensitivity-scaled, optionally Y-inverted **mouse-look delta** that swallows the first-frame cursor jump by baselining on (re)attach. **`plugins/gamepad`** (`gpinput::api()`, shared `gamepad.h`) provides rebindable button actions, trigger-as-button bindings (normalized `[0,1]` past a configurable threshold), single-axis bindings (dead-zoned for sticks, normalized for triggers, scalable/invertible), and **dead-zone handling** done right — a **radial** dead-zone on each stick (gate + rescale on the 2D magnitude) so a diagonal push is never clipped per-axis. Both fill their API table at `voxel_plugin_init` and register nothing else. Tests in `tests/InputPluginsTest.cpp` (18 tests) drive both through fake `RawSource` adapters — action held/edge transitions, rebinding moving the trigger, two-key axis cancellation, unknown-action inertness, mouse delta baseline/scale/invert, gamepad disconnect inertness, trigger threshold crossing, radial-deadzone gating and diagonal preservation, axis rescale+scale, and normalized triggers — so the whole input boilerplate is covered with no GLFW or window. Demo wiring is deferred to the *Demo modernization pass* (defaults stay byte-identical here, mirroring how B1 shipped plugin + tests without a new demo)*
- [x] **Demo with a meaningful HUD (sanity-check C2):** rather than attempting a general UI/HUD plugin (enormous scope, unlikely to satisfy most games), build a demo that features a **real, non-trivial HUD** — health, inventory, minimap, or similar — proving the engine provides the hooks needed for game UI and teaching the pattern. This validates the rendering seam for overlays and serves as a reference for developers building their own HUD. Coordinate with the demo modernization pass — *shipped: a thin **2D overlay seam** on `BgfxRenderer` (`hudClear`/`hudText`/`hudFill`/`hudCells` + `hudCols`/`hudRows`, ARCHITECTURE §9) generalizes the existing crosshair/`setHudText` debug-text path into an **immediate-mode cell-grid draw list** the game rebuilds each frame — colored cells (`hud::attr` packs a 4-bit fg/bg), enough to compose a real HUD with no per-game renderer change, deliberately a floor not a UI framework. The reference is `demos/18-hud-and-controls`: a first-person walk/mine/build on the material-showcase hardness strata that **field-tests all three M17 reference plugins at once** — player input flows entirely through the `keyboard-mouse` + `gamepad` plugins (C1; host installs a `RawSource` GLFW adapter, active device auto-switches when you pick up a controller), the player is a `kinematic-body` body (B1; the host sets wish-dir/jump, the engine's `register_on_tick` steps gravity + sweep-collision via `engine.update`, and the host derives **fall damage** from the landing speed). The HUD composes four classic elements over the seam — a **health bar** (`hudFill`) that depletes on hard landings and regenerates while grounded, an **inventory hotbar** (`hudText`, one colored slot per material) that banks mined voxels and spends them placing blocks, a **minimap** (`hudCells`) drawn as a top-down **excavation map** so dug-out columns read as pits, and a **status line** with coordinates, active device, and live fps from `Engine::getMetrics` (D2). The three reference plugins are compiled into the demo binary (each behind its `*_COMPILED_IN` guard) so their inline-static `api()` singletons resolve in one address space — the same wiring their tests use. All 455 tests stay green; the seam is mesher/persistence-free (pure renderer convenience)*
- [x] **Document save-format versioning contract (sanity-check B3):** `.vxc` carries a version header, but the forward-migration path as the engine evolves past 1.0 is undocumented. At minimum, document the versioning contract (what changes bump the version, what the engine does when it encounters an older/newer version) so consumers can plan around it — *shipped: `docs/save-format-versioning.md` documents the engine-owned `.vxc` format — the `VXCK` header + versioned palette/RLE layout, the version history (v1 M5 → v2 `light_emission` at M17, the one and only layout change to date, versioned correctly), and the contract proper: **what bumps the version** (any byte-layout change, including the common case of appending a `MaterialProperties` field), **what the engine does on a mismatch** (exact-match required; older/newer files decode to `nullptr` → treated as a cache miss and regenerated from the recipe locally, dropped on the network path — safe but edit-dropping), and the **forward-migration path** to commit to past 1.0 (a per-version decode ladder that up-converts older append-only files by defaulting new fields, refuses newer files with a clear diagnostic, and never reuses a version number). ARCHITECTURE §11 gains a "Save Format and Versioning" subsection linking it*
- [x] **Configuration guide (sanity-check D3a):** write a single reference doc cataloging every tunable knob the engine exposes — LayerConfig YAML fields, `Tuning.h` constants, and per-subsystem runtime APIs (`setTargetFrameRate`, `SoundParams`, etc.) — with descriptions, defaults, and when you'd change each. Pure discoverability; no code change — *shipped: `docs/configuration-guide.md` catalogs every knob across the engine's three configuration surfaces, framed by how/when each is set: **(A) world shape** — the `LayerConfig` YAML (per-layer `name`/`voxel_size_m`/`mode`/`decompose_to`/`interactive`/`chunk_size_voxels`/`view_distance_chunks`/`resident_chunk_budget`/`decompose_distance_m`/`streaming_volume`, the stack-wide validation rules, and a worked example); **(B) engine model & budgets** — every `Tuning.h` `constexpr`, by subsystem namespace (decomposition/streaming/physics/thermal/fluid/lighting/ao) with defaults and the per-frame-budget pattern explained; **(C) runtime APIs** — `Engine` loop/wiring + `getMetrics`, `Log` levels/handler, audio `SoundParams`/`EmitterParams` + `AudioManager`, renderer camera/viewport, `NetworkManager` server/client/interest, and the per-material `MaterialProperties` fields. Each entry carries a description, default, and "when you'd change it." Flags the D3b promotion (Tuning→runtime) as the doc's pending update. No code change*
- [x] **Promote `Tuning.h` compile-time constants to runtime (sanity-check D3b):** the per-frame budgets in `Tuning.h` (decomposition throughput, physics event caps, fluid/thermal cell limits, hysteresis) currently require a rebuild to adjust. Move them to runtime-settable values (via a global engine config struct or LayerConfig extension) with the current constants as defaults. Enables in-game quality sliders for players on different hardware, and eliminates the rebuild-per-tweak cycle during development. The configuration guide (D3a) should be updated to reflect the new runtime API once this lands — *shipped: new PUBLIC header `include/core/EngineConfig.h` defines `struct EngineConfig` — the twelve per-frame work budgets (decomposition load/decomp/apply, streaming hysteresis, physics aggregate-recompute/structural-event/support-flood caps, thermal cells, fluid cells/events, lighting cells/events) — plus a process-global `engineConfig()` accessor and `resetEngineConfig()`. Each subsystem (`DecompositionManager`, `LODManager`, `PhysicsSystem`, `PropagationSystem`, `ThermalSystem`, `FluidSystem`, `LightingSystem`) now reads its budget from `engineConfig()` each tick instead of the compile-time constant. `Tuning.h` keeps the named constants as the documented baseline but **derives them from `EngineConfig{}`'s defaults** (single source of truth) and includes the new header; the model constants (support span, stability factor, attenuation, AO, ambient floors) stay compile-time. Every field defaults to its prior value, so a consumer that never calls `engineConfig()` is byte-identical to the pre-D3b build — all 455 prior tests stay green. New `tests/EngineConfigTest.cpp` (3 tests) pins defaults==baseline, reset, and end-to-end runtime override through `LODManager` eviction hysteresis (458 total). `docs/configuration-guide.md` updated: surface B is now "model constants" (rebuild-only), and a new §C "Per-frame work budgets" subsection documents `engineConfig()` with the full constant↔field mapping*
- [x] **Example plugin suite covers all major hook types** — *shipped: audited every `PluginContext` extension point against the plugins that demonstrate it and found eight first-class hooks with **no example anywhere** — `register_noise`, `register_on_thermal_event`, `register_on_lighting_event`, `register_light_source`, `register_on_chunk_created`/`register_on_chunk_evicted`, `register_exporter`, and `register_interest_filter` (the design audit flagged five; the M17 lighting hooks and the interest filter widened the gap). New `plugins/example-hooks/` is a deliberate teaching catalog of exactly those: a deterministic world-sampled ridged `NoiseFn` (with a recipe `scale` param, via the `voxel_rng`/`recipe_param` ABI helpers), thermal/lighting/chunk-lifecycle/interest callbacks that thread plugin state through `user_data`, a registered point light source, and a real `.ehv` `ExporterFn` honoring the malloc-and-engine-frees contract — each section independent and copy-pasteable, heavily commented as the reference a developer reads to see the wiring. It links zero engine symbols like every plugin and builds both as a runtime `example-hooks.dll` and (guarded `EXAMPLE_HOOKS_COMPILED_IN`, ABI-stamp suppressed) compiled into the test binary. New `tests/ExampleHooksTest.cpp` (2 tests) wires it in via `wireInPlugin` and asserts every hook lands in its `PluginManager` registry under the plugin's ownership, that unload tears them all back down (the §8 owner-teardown contract), and that invoking each callback the way its subsystem would runs it end-to-end (pure normalized noise, observers recording through `user_data`, the exporter emitting the `.ehv` header+grid). With these, every major hook type is now demonstrated by at least one plugin. 460 tests green*
- [x] **Demo modernization pass — the demos are the teaching ladder:** the demo set is meant to be a *progressively more complex* sequence a developer reads in order to absorb the engine's conventions, so each demo must reflect **current** best practice, not the milestone-era pattern it was first written against. Several have drifted: demos 05/09/11 hand-roll the approach→drain→evict cascade with `DecompositionWorker` directly, even though M10's engine-owned `DecompositionManager` exists to do exactly that (only demo 10 uses it); demos 05/07/09/11/13 hand-stream immutable layers as "generated once, fully resident", the pattern M16's L5 supersedes (immutable meshes now stream under a `StreamingVolume` + `resident_chunk_budget`); and `setVerticalBand` users predate the general M16 streaming volume. Audit every demo against the post-M16 engine, migrate each onto the engine-owned facility it should be showcasing (`DecompositionManager`, volume-based residency, the gravity/axis seams where relevant), and verify the ladder still reads cleanly — each demo introducing one new concept on top of the last. The two new M16 demos (*Beyond blocks*, *Asteroid belt miner*) anchor the modern end; this pass brings the earlier rungs up to the same standard. Defaults stay byte-identical, so this is a teaching/clarity refactor, not a behavior change. As part of this pass, add proper leveled logging (`Log::info`/`Log::debug` with category tags) to each demo, replacing raw `std::cout` status prints — the demos should showcase the structured logging infrastructure (M17 D1) alongside the other engine-owned facilities they teach — *shipped: **leveled, category-tagged logging in all 18 demos** (raw `std::cout`/`cerr` → `Log::info`/`warn`/`error` under a per-demo category; the only surviving `std::cerr` is demo 06's custom export warn-handler closure, which IS the log sink). **DecompositionManager migration:** demos **05** (layered blocks→terrain + immutable backdrop) and **09** (recipe-built) now drive the engine-owned `DecompositionManager` exactly like demo 10 — it owns the composite cascade AND streams the immutable layer under its own `StreamingVolume` + `resident_chunk_budget` (the demo just applies the per-tick diff). Demo 09 was **rebuilt around what a composite-recipe demo should teach today**: dropped the old runtime `cave_density` toggle (it required demo-owned recipe resolution the manager intentionally does not expose), and instead teaches declarative composition (dig in to read interior/soil-cap/caves/ore in section) plus a **cascaded seed parameter shaping the recipe spatially** — the cave feature reads the engine-cascaded `__altitude` so caves thicken with depth, deterministically (recipe-world plugin's `cave_feature` gained the depth ramp; an explicit `cave_density` still pins a flat threshold for the cascade tests). **Immutable-layer budgets:** demo **07** (arena) immutable foundation/ramparts/props and demo **11** (shared world) immutable backdrop now carry a `resident_chunk_budget` and shed farthest-first (M16 L5). **Deliberately kept front-end-owned** (documented in-code, not regressions): demo 11's composite decomposition (its terrain holds replicated/persisted player edits the generic manager apply would overwrite — the networking interplay IS the lesson; a manager chunk-substitution hook could close this, noted as a follow-up), demo 07's terrace decomposition (README scoped it to immutable-streaming only; its detail layer holds demo-owned key/goal model chunks + save-substituted edits), and demo 13 (a fixed 32 m diorama — single chunk per layer, no streaming, so it got logging only). `setVerticalBand` is retained where appropriate (it is the box-volume Y-clamp convenience, still current for heightmap/slab worlds incl. demo 10). All 455 tests stay green*
- [x] **Demo for multi-level upward propagation (gap audit G1 follow-up):** the multi-level ancestor-chain collapse implemented this milestone is currently exercised **only** by `tests/MultiLevelPropagationTest.cpp` — no demo shows it. The two relevant demos each have just one of the two needed ingredients: demo 13 (*structural-collapse*) runs `PhysicsSystem` but over a **single** composite level (`blocks 2 m → terrain 1 m`), and demo 10 (*drill-to-the-core*) has a genuine deep chain (`continental 64 m → regional 16 m → local 4 m → terrain 1 m`) but never constructs a structural system. Build a demo that combines them: a deep composite stack + `PhysicsSystem` + the `crumble` response plugin, where mining fine terminal voxels under a large overhang hollows out a coarse macro until the **grandparent** caves in and cascades to neighboring macros — the emergent multi-scale collapse the engine's design sells, and the behavior that previously had no visible home. **Preferred: a new dedicated demo** (it teaches one crisp concept and slots into the ladder near demos 10/13); folding it into the Mega Demo is the **less-likely** fallback if a standalone scene proves redundant. Either way, coordinate with the *Demo modernization pass* above so the new rung uses the engine-owned `DecompositionManager` + volume-based residency rather than hand-rolled streaming — *shipped: new dedicated demo `demos/19-multilevel-collapse` realizes `tests/MultiLevelPropagationTest.cpp`'s `macro 4 m → micro 2 m → grid 1 m` stack (two composite levels — demo 10's depth) as demo 13's proven doubly-anchored stone bridge between two immutable bedrock towers, driven by `PhysicsSystem` (demo 13's structural system). The player mines the **1 m grid grandchildren** of a deck macro near one tower (HOLD LMB chews one grid voxel at a time via `RemovalAccumulator`; RMB knocks out a whole 4 m macro at once); the engine's **recursive aggregation** walks the damage up grid→micro→**macro**, the 4 m grandparent's aggregate falls until it caves, and the grandparent cascade marches outward and **terminates at the macros still anchored to the far tower** — the exact `MacroCollapseCascadesAndTerminates` behavior, now flyable. **Key design choice (documented in the demo header):** the support flood runs independently at each composite level with span measured in *voxels at that level*, so the same material reaches a shorter **metric** distance at a finer level — a macro-stable structure is micro-unstable, and a response acting at every scale (the generic `crumble` plugin) would let the micro level self-destruct the deck. So the demo's response — registered in-process via `PluginManager::wireInPlugin`, the mechanism the test uses — filters to the **coarsest (grandparent) scale** and clears the unstable macro's full grid footprint (the test's `macroCrumble`, productized), making the grandparent the sole structural actor and the grid→micro→macro aggregation chain the thing driving collapse. A HUD line surfaces grandparent collapses vs. micro-scale events detected-but-not-actioned, making the upward chain visible. Like its sibling demo 13 this is a **fixed structural diorama** (hand-built scene + `PhysicsSystem`), not a streamed/decompose-on-approach world, so — exactly as the modernization pass exempted demo 13 — `DecompositionManager` is inapplicable here (there is no recipe-generated terrain to stream; the bridge must be a known, controlled structure for the collapse to be predictable and teachable); residency is the explicit per-layer chunk loading the structural detection needs. New `tests/MultiLevelCollapseDemoTest.cpp` (3 tests) pins the demo's scene: stable-as-built at the macro scale, the grandparent cascade stopping at the far-anchored remnant, and engine-never-writes with no response — also the first coverage of `PropagationSystem::isAnchor`'s **immutable-layer** anchor path at every level of a deep stack (only single-level demo 13 exercised immutable anchors before). All 18 prior demos and the structural test suites stay green*
- [x] **Surface-normal camera orientation (renderer):** the renderer's view is currently hard Y-up — `BgfxRenderer::render` builds the view matrix from pitch/yaw with an implicit `(0,1,0)` up and ignores the `roll` it already stores, so on an arbitrary surface the horizon looks tilted even when the kinematics are correct. This is the visual counterpart to M16's L2/L7 gravity-relative grounding: once "down" can point any direction, the camera should be able to align its up-axis to an arbitrary surface normal (the local `-gravityAt` up). Thread a full camera basis (or honor `roll` + a supplied up-vector) through `setCameraRotation`/the view matrix so a player standing on the +X face of an asteroid sees a level horizon. Surfaced by the *Asteroid belt miner* demo, which works mechanically but documents this as a known visual caveat; it is also the natural home for the G3 +Y-up shade-ramp fix (`docs/m16-generality-audit-m15-followup.md`), the other renderer-tier axis assumption. Default to the current Y-up basis so every existing demo renders byte-identically — *shipped: a new public `Renderer::setCameraUp(worldUp)` seam (default `(0,1,0)`) lets a game point the camera's up-axis at any surface normal. The shared header-only `cameraBasis()` (`include/renderer/CameraBasis.h`) builds the canonical +Y forward/right/up from pitch/yaw + roll, then rotates that frame so +Y maps onto the supplied up; with the default up and zero roll the rotation short-circuits to the identity, so `BgfxRenderer::render`'s historical float path is kept verbatim and **every existing scene's view matrix is byte-for-byte unchanged**. The same basis feeds `Frustum::update` (now taking the up vector + roll) so culling planes rotate with the view. **G3 shade-ramp fix:** `buildChunkMeshData`'s directional shade ramp now resolves each face through `axisrole::roleOf` against the gravity vector — the same seam the textured face roles (G1) and recipe boundaries (G2) use — so the fake-lighting follows local "up" instead of always lighting +Y (default −Y byte-identical). **Demo:** *Asteroid belt miner* (17) wires it up — in the suit the camera aligns to `-gDir` and its mining raycast/movement derive `look`/`right` from the same `cameraBasis`, so the horizon is level and the crosshair stays locked to what is mineable on any face; the jet and zero-g keep Y-up. ARCHITECTURE §9 gains a "Camera Orientation" subsection and §18 lists the camera/shade as further consumers of the gravity vector. New `tests/CameraBasisTest.cpp` (6 tests: byte-identical Y-up default, level horizon on +X, pitch-0 tangency for arbitrary ups, orthonormality, antipodal, roll) and `tests/ChunkMeshDataTest.cpp` gains gravity-aware shade tests; 471 tests green*
- [x] **Distance obscurance — sell LOD emergence with mist/dust/light (renderer + plugins):** the decomposition cascade now stages coarse→fine at decoupled distances (the *Asteroid belt miner* demo refines a body's 4 m silhouette at 280 m and its 1 m mineable grid only within 90 m), but every layer is still drawn at full clarity to the far clip, so the coarse→fine transition and the chunk-load boundary at view-distance edge are baldly visible — detail visibly *snaps* into place rather than resolving. Add a distance-obscurance depth cue so geometry emerges out of murk as it refines: a renderer seam for atmospheric falloff (fog color + near/far density, applied in the chunk shader) driven as a *policy* — mirroring how gravity (L7) and authority (§15) are policies, not baked engine forces — plus a small **mist/dust** content plugin and a **basic lighting/attenuation** plugin that supply and animate those parameters (e.g. a dust haze in the asteroid field, range-attenuated light in a cave). Tuning the obscurance distance to sit just inside each layer's decompose distance hides the pop entirely. Default to no fog (density 0) so every existing demo renders byte-identically. Complements, not replaces, the decompose-distance tuning — concealment plus earlier decomposition together is what makes streamed worlds read as continuous. Surfaced by the *Asteroid belt miner* demo — *shipped: a new public `Renderer::setFog(FogParams)` seam (`include/renderer/Fog.h`) takes a fog color + near/far band + max density. The chunk shaders apply it as pure mechanism: `vs_voxel.sc` outputs the view-space distance (`length(mul(u_modelView, position))`) while leaving `gl_Position` computed exactly as before, and `fs_voxel.sc` blends the shaded fragment toward the fog color by a linear near→far ramp clamped to `[0,1]` and scaled by density (RGB only — translucency is preserved). `BgfxRenderer` pushes the band through two `Vec4` uniforms (`u_fogColor`/`u_fogParams`) each frame from a `FogParams` policy member. **`FogParams.density == 0` (the default) disables fog** — the shader's `mix()` collapses to the un-fogged fragment exactly, the uniforms default to zero, and `fogFactor()` (the CPU mirror of the shader curve) returns 0 — so every existing scene is byte-identical (committed shader bytecode regenerated for all five backends). The color/band/animation are **policy**, supplied per-frame by the game like gravity (L7) and authority (§15): two pure-policy reference plugins ship, each filling a shared `api()` table at init and registering no engine hooks — **`plugins/atmospheric-mist`** (a dust haze fading to the sky color whose peak density slowly *breathes*, for open fields) and **`plugins/range-attenuation`** (a flickering lit radius fading to near-black — the "you can only see so far in here" cave/torch look — riding the same fog mechanism with a dark target color and a band equal to the light's reach). The host calls `api().sample(t)` each frame and forwards the result to `setFog`. A companion `Renderer::setClearColor(rgb)` seam sets the background color so the fog color can match it — far geometry then dissolves seamlessly into the background instead of leaving a halo at the far plane (default `0x303030`, byte-identical). *Asteroid belt miner* (17) wires up atmospheric-mist with its band tuned to the macro decompose distance (80–260 m, bodies are ~160 m apart), so a body is already deep in dust by the time its coarse 4 m silhouette appears and the LOD pop resolves out of the murk; the dust + background share one warm-gray tint so the far plane is invisible, and an **`M` toggle** drops fog back to the stark void so the pop it conceals is directly A/B-able. ARCHITECTURE §9 gains a "Distance Obscurance" subsection; `docs/configuration-guide.md` documents the renderer seams. New `tests/FogTest.cpp` (9 tests) pins the `fogFactor` curve (incl. the byte-identical density-0 case and a degenerate band) and both plugins' supply/animation (defaults, density breathing within `[0,1]`, configure override, flicker keeping the band valid); 486 tests green*
- [x] Performance profiling pass on decomposition, chunk management, and rendering — *shipped: a **measure-then-fix** pass. Added a reusable, **disabled-by-default** CPU zone profiler (`include/core/Profiler.h` + `src/core/Profiler.cpp`: a `VOXEL_PROFILE_SCOPE` RAII timer that reads enablement once and, when off, never touches the clock or registry — so instrumented code is byte-identical and effectively free in normal runs; thread-safe for the decomposition workers, locking only while enabled) and a **headless** benchmark harness (`bench/profile_pass.cpp`, target `profile-pass`: no window/GPU, drives a 4-level solid composite cascade for decomposition + streaming churn, and the CPU mesher over representative 32³ chunks). Instrumented the three named areas — `decomp.tick`/`decomp.approach`/`decomp.stream`/`decomp.immutable` (decomposition + chunk management), `stream.desired` (residency-set rebuild), `mesh.build` (rendering). **Findings** (`docs/m17-performance-profiling.md`): a decomposition tick costs ~0.18 ms avg / ~1.6 ms peak — well inside a frame, with the per-frame `engineConfig()` budgets preventing hitches by construction; the approach-trigger scan is the dominant decomp phase but AABB-pre-rejected and budget-capped; `stream.desired` is sub-µs for box volumes. **The one justified fix is in rendering:** `ChunkMesh::build` allocated fresh output vectors for *every* chunk it meshed; it now reuses `thread_local` scratch (the mesher already `.clear()`s at entry, so capacity is retained and steady-state builds never reallocate), measured to cut CPU mesh-build time **~25% (light chunks) to ~46% (dense chunks)** with **byte-identical** geometry (all 490 tests green, incl. the geometry-pinning mesh suites; +4 new `tests/ProfilerTest.cpp`). A speculative mesher `reserve()` was implemented, measured to be a wash/net-loss (a fixed estimate under- or over-reserves), and **reverted** — the pass doing its job. View-frustum culling (A7) validated as already-shipped; the §11 byte-budget memory backstop (B4) recorded as a deliberate non-change (the per-layer `resident_chunk_budget` chunk-count cap is the operative, measured-sufficient bound). ARCHITECTURE §9 notes the mesh-build buffer reuse
- [x] **Standard skybox support (renderer):** evaluate whether the renderer needs a first-class skybox seam — a cubemap or procedural sky, distinct from the flat clear-color + distance-fog policy already in place (§9) — and add it if the evaluation finds a gap. Needed groundwork for the planned M19 "No Man's Voxel" demo, where the player leaves a world's local bounds and needs a convincing sky/space backdrop rather than the flat clear color. — *evaluated (`docs/m17-skybox-evaluation.md`): the gap is real — flat `setClearColor` + fog-toward-that-color can only make the void **one** color; they cannot paint a horizon, a zenith→horizon gradient, or a distinct space backdrop, which is exactly what M19's fly-out-to-space demo needs. **Shipped a procedural-sky seam** (mechanism-vs-policy, like fog §9). Mechanism: `Renderer::setSky(SkyParams)` (new PUBLIC `include/renderer/Sky.h`) carries a **zenith/horizon/ground** color triple + a horizon-band `falloff`; `BgfxRenderer` paints a **camera-centered gradient sphere** whose per-vertex color is `skyColor(dir, up)` measured against the **camera up** (`setCameraUp`), so the horizon stays level on any surface (the §18 gravity vector). Deliberately procedural, **not** a cubemap: it needs no texture-asset pipeline and **no new shader** — the sphere is drawn through the existing `vs_voxel`/`fs_voxel` program (samples the 1×1 white tile, baked vertex color passes through), so the committed shader-bytecode set is untouched. Correct **by depth test alone** (eye-centered sphere → one forward hit per ray; drawn at `0.999·farClip` with `DEPTH_TEST_LEQUAL`, no depth write → passes on empty pixels, rejected behind geometry, order- and cull-independent); fog forced off for the sky submit. Policy: the reference **`plugins/procedural-sky`** supplier (pure-policy, registers no engine hooks — the fog-supplier shape) ships `daySky()`/`duskSky()`/`spaceSky()` presets + an optional day/night cycle. **Default `SkyParams::enabled == false` draws no sky → every existing scene is byte-identical** (flat clear color shows through; a host that never calls `setSky` is unchanged). New `tests/SkyTest.cpp` pins the gradient (stops hit exactly along the up axis, tracks camera up, degenerate-input-safe, monotone between stops) and the plugin (defaults, static-by-default, dark space preset, animated cycle, configure). ARCHITECTURE §9 gains a "Sky / Background" subsection; `docs/configuration-guide.md` documents the seam. Textured cubemap + sun/star layer recorded as follow-ons; demo wiring lands with the M19 demo this is groundwork for.*
...
**M18.5 — Composite Heightmap Worlds (Breakfix)**

> A small breakfix milestone slotted between M18 and M19, surfaced by the Mega-Demo (`docs/m18-mega-demo-metrics.md`, "A note on scope honesty"). The engine has **two worldgen paths that don't compose**: the *generator* path (a `LayerGeneratorFn`, or the M6 default-recipe decomposition that re-runs a child generator over the subvolume) can follow a heightmap but offers no declarative composition; the *recipe* path (M9, §6) offers material distribution, boundary caps, feature overlays, and the M10 seed-parameter cascade but models every macro voxel as a **fully-solid cube with face-relative skins** — `fillChildChunk` (`src/world/ResolvedRecipe.cpp`) fills every child cell, so a recipe refines a solid cube into a smaller-voxel solid cube and **cannot follow a surface**. A developer therefore cannot have *both* a rolling heightmap surface *and* recipe-driven composite decomposition — and because M13 structural collapse fires only for composite macro voxels (§7), a heightmap world built the only supported way (a single terminal layer, as the Mega-Demo is) has no composites and so can never show collapse. This milestone makes recipe decomposition **surface-aware**, implementing the long-deferred `docs/proposals/recipe-occupancy.md` (v1 + v2) plus an engine-derived coarse occupancy, then rebuilds the Mega-Demo onto a composite heightmap so M13/M14 and the recipe authoring model coexist. The change is additive and ABI-safe (zero-initialized = absent = byte-identical to today); existing recipe worlds (recipe-world/drill-world/asteroid) are unaffected.
>
> Design reference: `docs/proposals/recipe-occupancy.md` (promote from "proposed" to "scheduled — M18.5"). Determinism (§4) and the §13 worker/`PluginManager` boundary are non-negotiable, exactly as in M9/M10.

- [x] **Occupancy carving — recipe v1 (`recipe-occupancy.md`):** add an optional `OccupancyDesc` to `RecipeDesc` (`include/plugin_api.h`; appended, zero-init `present == false` ⇒ today's full-solid fill, so every existing plugin is byte-identical). A recipe references a height/signed-distance field by registered `noise_id` + `threshold`; the plugin registers that field once via `register_noise`. Thread it through the deep-copy value type (`src/world/Recipe.h`, `PluginManager`), resolution + validation on the main thread (`src/world/RecipeResolve.*`, fail-loud on an unknown id, same as a distribution noise), and a **carve gate** in `fillChildChunk` (`src/world/ResolvedRecipe.cpp`) run *before* the distribution sample: a child cell whose field value is below threshold becomes `Voxel::empty()`. Carve-only — never marks a cell solid outside the macro — so the coarse-supersets-fine invariant (§4) holds by construction. Seed it with a fixed `kOccupancySalt` decorrelated from the distribution/feature seed domains, and merge its params through the standard M10 precedence (inherited ancestor seed params → recipe `seed_parameters` → occupancy's own params), so the reserved `__altitude`/`__parent_material` keys are available. Tests: carve determinism (same seed ⇒ same holes), superset invariant (no solid child outside a solid parent), absent-occupancy byte-compatibility, threshold edge cases. — *shipped: `OccupancyDesc { noise_id, threshold, params, param_count, present }` appended to `RecipeDesc` (plugin ABI version bumped 2 → 3, since a cell's `present`/threshold is read from the plugin's struct); owning `OccupancyValue` mirror deep-copied in `Recipe::fromDesc`; `ResolvedOccupancy` resolved in `resolveRecipe` (empty id ⇒ built-in `value`, seed-merged params) and validated in `validateRecipes` (unknown id is a hard startup error). The carve gate in `fillChildChunk` runs before the distribution sample, sampling the field at the child voxel's **world** center (seamless across macros) under a fixed `kOccupancySalt`; absent or null-fn occupancy is byte-identical to the pre-M18.5 fill (the `center` computation was hoisted above the gate and reused by the distribution, no double-sample). New `tests/RecipeOccupancyTest.cpp` (7 tests: absent = fully solid, present-but-null fail-soft, carve determinism, carve-only/survivors-unchanged, threshold ≤ min ⇒ all solid, threshold > max ⇒ all empty, and a heightmap field producing a clean per-column sloped surface); full suite 501 green, whole solution builds clean.*
- [x] **Surface-relative boundary cap — recipe v2 (the cap the Mega-Demo needs):** add a `surface` mode to `BoundaryDesc` where `depth` is measured **downward from the topmost solid cell of each carved column**, not from the macro's geometric face — a worker-only second pass in `fillChildChunk` (per-column, after the carve + distribution), gated by a mode flag on `BoundaryDesc` (no ABI impact beyond the flag). This is what lays grass-on-surface / dirt-subsoil / stone-below correctly over a sloped terrain, instead of a macro-face cap that lands buried or floating in air on a hillside. Resolve "up" against the same `gravityDir` the existing top/bottom/side roles use (§6/§16-G2), so a radial well caps correctly too. Tests: a cap tracks a sloped carved surface; default (macro-aligned) mode byte-identical to today. — *shipped: `BoundaryMode { MacroFace, Surface }` appended to `BoundaryDesc` (threaded through `BoundaryValue`/`ResolvedBoundary`); the main fill loop paints only `MacroFace` boundaries inline, and a per-column second pass in `fillChildChunk` applies `Surface`-mode `top`/`bottom` caps — walking each column from the surface end inward off the resolved gravity axis, a solid cell whose outward neighbor is air starts a `depth`-cell repaint band. Reduces to `MacroFace` when there's no carve (the "surface" is the macro face); `side` is not surface-capped; documented one-chunk under-cap seam only when a macro spans several child chunks vertically. 4 new tests (slope-tracking cap, depth band, no-carve == macro-face, surface+macro-face independence). Same ABI version 3.*
- [x] **Engine-derived coarse occupancy (the duplicate-height-field footgun):** today a composite layer's world-load occupancy comes from a *hand-written* `LayerGeneratorFn` that must conservatively mark a macro present wherever the fine surface rises into its span (the §4 max-over-footprint authoring contract — the exact rule `layered-world`'s `blocks_generator` got wrong with a center sample). Since the coarse occupancy and the fine carve now reference the **same** registered height field, let the engine derive the coarse occupancy from it directly — max over the macro footprint at the child resolution — so the height field lives in exactly one place and the superset invariant is guaranteed, not authored. Keep the hand-written generator path working (opt-in: a composite layer with an occupancy-bearing recipe and no registered generator uses the derived occupancy; one with a generator keeps it). Update ARCHITECTURE §4/§6 to as-built; flip `docs/proposals/recipe-occupancy.md` to "implemented." — *shipped: `DecompositionManager` synthesizes a `DerivedOccupancyGen` at construction for each root composite layer that has an occupancy recipe and no registered generator. Its `LayerGeneratorFn` (`derivedOccupancyGenerator`) marks each macro present iff sampling the carve field over the macro's child-resolution footprint — with the **same** per-macro occupancy seed (`voxel_seed_mix(decompSeed, kRecipeOccupancySalt)`, the salt now shared from `ResolvedRecipe.h`) and identical world positions — finds any solid cell, so coarse occupancy is an **exact** superset of the macro's own decomposition (early-out on first solid). The atomic macro carries the recipe's dominant interior material. A registered generator wins (opt-in). ARCHITECTURE §4 (new "Engine-derived coarse occupancy") and §6 (new "Occupancy Carving" + surface-cap notes) updated; proposal flipped to "implemented". 2 new tests (occupancy tracks the carve surface incl. straddle-superset; registered generator wins). Full suite 507 green; whole solution builds clean.*
