...
/**
 * @file asteroid_field.h
 * @brief Shared asteroid-field geometry — the single source of truth for WHERE the bodies are.
 *
 * Included by BOTH halves of the "Asteroid belt miner" demo (M16):
...
/**
 * @brief One asteroid resolved from its cell.
 *
 * `seed` is the per-body RNG state AFTER the placement draws, so the detailed
 * generator's relief noise (which seeds from it) decorrelates body-to-body.
 */
struct Body {
    glm::dvec3 center{};
    double     radius = 0.0;
...
/**
 * @brief Deterministic per-cell seed.
 *
 * Chains the header's splitmix mixer over the cell index so neighbouring
 * cells decorrelate.
 */
...
/**
 * @brief Resolve the asteroid (if any) hosted by cell (gx, gy, gz).
 *
 * The draw ORDER (fill, jitter×3, radius, icy) is the field's contract — both
 * generators and the gravity policy depend on it, so it must not change
 * without regenerating the world.
 * @return false for an empty cell.
 */
...
/**
 * @brief Invoke fn(const Body&) for every asteroid whose home cell is in the 3×3×3 neighborhood of world point p.
 *
 * That neighborhood is exact (see kRadiusMax).
 */
template <class Fn>
...
/**
 * @brief The nearest asteroid CENTER to world point p (by center distance), the "down" target for the radial gravity well.
 *
 * Center, not surface: a uniform body's field points at its center.
 * @return false when no body is near enough to be in the 3×3×3 neighborhood —
 * open space, where the demo lets the suit drift.
 */
...
/**
 * @brief Conservative, NOISE-FREE solidity test for a coarse (composite-layer) voxel.
 *
 * Is the axis-aligned cell of side `vs` centered at `cellCenter` within the
 * MAXIMAL surface radius of any nearby body? Uses the exact cube-vs-sphere
 * overlap test (distance from the body center to the nearest point of the
 * cube), which is monotonic under cell nesting — so a solid fine voxel's
 * parent macro is always solid too (the coarse-supersets-fine invariant a
 * decomposition cascade needs). Reports the covering body's `icy` trait
 * (@p outIcy) so the coarse crust can be ice or rock.
 */
