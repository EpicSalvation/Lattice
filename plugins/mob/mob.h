...
/**
 * @file mob.h
 * @brief Shared API for the mob reference plugin (M18 mega-demo).
 *
...
/// @brief AI state, surfaced so the host can tint/animate the rendered body per state.
enum class State : uint8_t { Wander, Chase, Attack, Dead };

/// @brief Post-step pose of one mob, valid after the tick hook fires.
struct MobState {
...
/// @brief Singleton accessor. Plugin fills it during init; host reads it after wiring in.
inline API& api() {
    static API instance;
    return instance;
}
