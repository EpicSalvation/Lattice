 ...
 * @brief Shared API for the keyboard-mouse reference input plugin (M17 C1).
 *
...
/**
 * @brief Raw hardware poll adapter the host installs.
 *
 * The plugin pulls through these each frame; the host implements them as
 * one-line wrappers around GLFW (the host owns the window). `user` is passed
 * back to each callback unmodified (typically the GLFWwindow*).
 */
struct RawSource {
 ...
 *
 * The plugin fills this during init; the host reads it after loading the
 * plugin. Single-writer (plugin init) / single-reader (host loop).
 */
inline API& api() {
    static API instance;
    return instance;
