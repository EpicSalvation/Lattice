...
/**
 * @file gamepad.h
 * @brief Shared API for the gamepad reference input plugin (M17 C1).
 *
...
/// @brief Standard-mapping axis indices (== GLFW_GAMEPAD_AXIS_*), for stick() side and readability at call sites that don't include GLFW.
enum Axis : int {
    AxisLeftX        = 0,
    AxisLeftY        = 1,
    AxisRightX       = 2,
...
/// @brief Which stick stick() reads.
enum Stick : int { StickLeft = 0, StickRight = 1 };

/**
 * @brief Flat snapshot of one gamepad, filled by the host from glfwGetGamepadState.
 *
 * Field layout matches GLFWgamepadstate (buttons then axes) but the host
 * copies values across, so no GLFW type is named here. axes are [-1,1];
 * sticks rest at 0, triggers rest at -1.
 */
struct GamepadState {
...
/**
 * @brief Raw poll adapter the host installs.
 *
 * The plugin calls poll() each update for the configured joystick id; the
 * host wraps glfwJoystickIsGamepad + glfwGetGamepadState (leaving
 * connected == 0 when absent).
 */
struct RawSource {
...
/// @brief Global function-pointer table filled by the plugin at init.
struct API {
 ...
 *
 * The plugin fills this during init; the host reads it after loading the plugin.
 */
inline API& api() {
    static API instance;
    return instance;
