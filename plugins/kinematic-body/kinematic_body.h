...
/**
 * @file kinematic_body.h
 * @brief Shared API for the kinematic-body reference plugin (M17 B1).
 *
...
/**
 * @brief Configuration for a new body.
 *
 * All fields have sensible defaults matching the engine's existing demo
 * conventions (0.6×1.8×0.6 player, 25 m/s² gravity, 8 m/s walk, 9 m/s jump).
 */
struct BodyDesc {
    WorldCoord center;
...
/**
 * @brief Per-frame input for a body.
 *
 * The host computes the wish direction from camera yaw and key state and
 * passes it here; the plugin applies walk_speed and integrates gravity.
 */
struct BodyInput {
...
/// @brief Post-step body state, valid after the tick hook fires.
struct BodyState {
 ...
 *
 * The plugin fills this during voxel_plugin_init; the host reads it after
 * loading the plugin. Thread-safe for the single-writer (plugin init) /
 * single-reader (host main loop) pattern the engine uses.
 */
inline API& api() {
    static API instance;
    return instance;
