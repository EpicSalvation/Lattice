...
/**
 * @file EngineConfig.h
 * @brief Runtime-adjustable engine configuration (M17 D3b).
 *
...
/// @brief Per-frame work budgets for the engine's streaming/decomposition/simulation subsystems.
struct EngineConfig {
...
/**
 * @brief Process-global runtime config.
 *
 * Defaults to the baseline above; mutate it for a quality slider or dev-time
 * tuning. Read by the simulation subsystems each tick.
 */
EngineConfig& engineConfig();

/**
 * @brief Restore every field to its default.
 *
 * A "reset to defaults" action for a UI, and the way a test that mutated the
 * config leaves it clean for the next test.
 */
