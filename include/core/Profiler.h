...
/**
 * @file Profiler.h
 * @brief Lightweight, always-compiled CPU zone profiler (M17 performance pass).
 *
...
/// @brief Accumulated timing statistics for one named profiling zone.
...
/// @brief Process-global, always-compiled CPU zone profiler singleton.
class Profiler {
public:
    ...
    /**
     * @brief Record one timed sample for `name`.
     *
     * Called by ProfileScope's destructor; safe to call directly for manual timing.
     */
    void record(const std::string& name, uint64_t ns);

    /// @return a snapshot of every zone, sorted by total elapsed time descending.
    std::vector<ProfileZoneStat> snapshot() const;

    /**
     * @brief Human-readable table (zone | calls | total ms | avg us | max us), sorted by total time descending.
     * @return the formatted report, or an empty string when nothing was recorded.
     */
 ...
 *
 * Construction snapshots enablement: when disabled the object is inert (no
 * clock read, no registry access), so instrumented code pays only a branch on
 * the common (un-profiled) path.
 */
class ProfileScope {
public:
