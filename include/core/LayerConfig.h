...
/// @brief How a layer's voxels are generated, edited, and persisted.
...
/**
 * @brief Shape of a layer's camera-centered streaming residency volume (M16, L1).
 *
...
/// @brief Declarative definition of one layer in a world's decomposition stack.
struct LayerDef {
    std::string              name;
...
/**
 * @brief Parses and validates a layer stack from a YAML project config file.
 *
...
public:
    ...
    /// @return a pointer to the named layer, or nullptr if it does not exist.
    const LayerDef* findLayer(const std::string& name) const;

    /// @return the 0-based index of the named layer, or -1 if it does not exist.
