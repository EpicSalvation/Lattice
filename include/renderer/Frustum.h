...
/**
 * @brief Conservative view-frustum test for chunk bounding spheres.
 *
 * Plane-distance tests against the bounding sphere err on the side of
 * "visible" — this never culls geometry the renderer would actually draw, it
 * only skips submitting chunks behind the camera or outside the view cone.
 */
struct Frustum {
    ...
    /**
     * @brief Recompute the frustum planes for the given camera state.
     *
     * worldUp / roll thread the surface-normal camera orientation (M17)
     * through culling so the frustum planes rotate with the view: default +Y
     * up and roll 0 reproduce the historical Y-up frame bit-for-bit
     * (cameraBasis short-circuits the rotation to the identity), so culling
