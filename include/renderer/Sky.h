...
/**
 * @brief SkyParams — the renderer's procedural-sky (background) seam (M17, docs/architecture.md §9; docs/m17-skybox-evaluation.md).
 *
...
/**
 * @brief The sky color for a view direction `dir`, with `up` the world-space up the gradient is measured against.
 *
 * `up` is typically the camera up / local -gravity. This is the EXACT curve
 * the renderer bakes into the background sphere's per-vertex colors, kept
 * here so the CPU side (tests, and the supplier plugins) can reason about the
 * same gradient the GPU shows. Neither vector need be normalized; a
