...
/**
 * @brief FogParams — the renderer's distance-obscurance (atmospheric falloff) seam (M17, docs/ARCHITECTURE.md §9/§18).
 *
...
/**
 * @brief The fog blend factor at view-space distance `dist` (metres), in [0, density].
 *
 * This is the EXACT math the fragment shader runs (shaders/fs_voxel.sc): a
 * linear near→far ramp clamped to [0,1] and scaled by the max density. Kept
 * here so the CPU side (tests, and the supplier plugins) can reason about the
 * same curve the GPU applies. With density 0 the result is 0 for every
 * distance → no fog.
 */
