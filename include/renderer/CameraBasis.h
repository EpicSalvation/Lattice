...
/**
 * @brief Orthonormal camera frame (forward/right/up) returned by cameraBasis().
 *
 * cameraBasis() builds this frame from pitch/yaw/roll interpreted in a frame
 * whose "up" is an arbitrary world-up vector (M17, surface-normal camera
 * orientation; docs/ARCHITECTURE.md §9/§18).
 *
...
/**
 * @brief Rotate `v` by the rotation that maps +Y onto the unit vector `u`.
 *
 * Identity when u == +Y (so callers that never leave Y-up are unaffected); a
 * 180° flip about X for the antipode (u == -Y); Rodrigues' rotation otherwise.
 */
...
/**
 * @brief Build an orthonormal camera frame from pitch/yaw/roll around an arbitrary world-up vector.
 * @param pitch   pitch in radians
 * @param yaw     yaw in radians
 * @param roll    roll in radians, about the forward axis
 * @param worldUp world-space up direction; need not be normalized
 * @return the resulting forward/right/up frame. See struct comment above for
 * the byte-identical default guarantee (worldUp == +Y, roll == 0).
 */
...
/**
 * @brief Rotate the unit vector `current` toward the unit vector `target` by at most `maxRadians`.
 *
 * Pure and stateless — the *rate* and the per-frame state are the game's to
 * own (this is the slerp step a game runs each frame to ANIMATE a camera-up
 * change instead of snapping it, e.g. when a player lands on a surface or
 * crosses between gravity bodies). When the remaining angle is within the
 * step it returns `target` exactly, so the animation settles cleanly; the
 * ~180° case (no unique rotation axis) falls back to a deterministic
 * perpendicular axis. A non-positive step leaves `current` unchanged.
 *
 * Typical use (mirrors cameraBasis's caller): feed the result to BOTH
 * cameraBasis (for the look/raycast direction) and Renderer::setCameraUp, so
 * the rendered view and the host's picking stay locked together through the
 * animation.
