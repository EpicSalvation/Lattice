...
| At most one layer flagged `interactive: true` (L4) | Hard error, layer names printed |
...
**Properties are read off the voxel, not looked up live.** Materials are copied into voxels by value at generation/import time (§8). A consumer therefore reads `voxel.material.<field>` directly; it does not consult the plugin material registry at simulation time. The accepted consequence: re-registering or unloading a material does **not** retroactively change voxels that already exist — their properties were frozen when they were created. The material-id registry lookup (`PluginManager::material()`) exists for *tooling* — importers, UI, build-menu selection — and is deliberately off every simulation hot path.

**Worked example — voxel-removal cost (`src/simulation/RemovalModel`).** The effort to remove a voxel is a pure, deterministic function of its `hardness` and an optional tool `power`:
...
`structural_strength` (structural collapse, §7) and `thermal_conductivity`/`porosity` (fluid and heat, §17) are consumers that follow this same contract: read the property off the voxel, respond to the value, stay off the material-id path.
...
**Reuse across the plugin ABI boundary.** Engine subsystems and in-tree demos call the functions in `src/world/Noise.h` directly (in-tree consumers may reach into `src/`, §12). Out-of-tree plugins, which link zero engine symbols (§12), participate two ways: they **provide** noise through `register_noise`, and they **consume** built-in/registered noise through `resolve_noise(ctx, noise_id) -> NoiseFn` on `PluginContext` — the §12 "add a function pointer when a consumer needs it" move, added (C2) when volumetric generators became the first non-recipe consumer. `resolve_noise` returns the registry's winning fn (a plugin `register_noise` of an id overrides the built-in floor) or `nullptr` for an unknown id, so a consumer fails loudly rather than silently mis-generating. The built-in floor (value/fbm/ridged/worley) is registered at `PluginManager` construction, so the resolver is usable from a plugin's `init` even in a host that never calls `Engine::init`. Built-in noise ignores its `user_data` argument, so the bare-`NoiseFn` accessor cannot carry a per-registration `user_data` (none of the built-ins need one). The in-tree `base-terrain` / `layered-world` generators consume the built-in `value` noise this way rather than a hand-rolled inline copy. The deterministic seed helper itself is inline in `plugin_api.h`, so it is usable everywhere with no resolver.
...
**Files:** `src/simulation/PropagationSystem.cpp/.h` (detection), `src/simulation/PhysicsSystem.cpp/.h` (per-frame driver + event firing), `src/core/Tuning.h` (`tuning::physics` knobs). The design below is the as-built contract.
...
Stability is evaluated at **macro-voxel (composite) granularity**, per the aggregation rule below — not per terminal voxel. The model is a **support-potential flood** (axis-free, so it does not bake in a "down" direction — generalized gravity is a separate concern (§18), and this model captures *support reach*, not gravitational load):
...
**Materials are copied into voxels by value** (`Voxel` holds a `MaterialProperties`, not a material-id reference). Unregistering a material therefore does not retroactively change voxels that already exist; the material registry is consulted only at generation time. A consumer that wants an unload to be *visible* must regenerate the affected chunks — the `03-plugin-driven-world` demo does this by dropping and re-streaming resident chunks whenever the plugin set changes.

### Feature Generators in the Terminal-Layer Path

Feature generators (`register_feature_generator`) post-process an already-filled voxel grid. In the full design they are referenced by composition recipes (§6); for the terminal-layer demo (`03-plugin-driven-world`) they are applied directly: after the base layer generator fills a chunk, each registered feature generator runs over it in registration order. The `water` plugin uses this to flood empty voxels up to a fixed sea level — additive, self-contained, and cleanly removable, since dropping the plugin removes its feature generator from the pass.
...
The directional **shade ramp** in the mesher is a third consumer of the up vector. `buildChunkMeshData`'s fake-lighting ramp (top brightest, bottom darkest, sides between) resolves each face's role through `axisrole::roleOf` against the same gravity vector that drives the textured face roles (G1) and recipe boundaries (G2), so the lit relief follows local "up" on an off-axis body instead of always lighting +Y (G3). Under the default −Y gravity `roleOf` is the identity, so the ramp is byte-identical to the historical +Y-up shading.

### Distance Obscurance (atmospheric fog)
...
### Sky / Background (procedural sky)

The clear color (above) and distance fog answer "what color is the void behind the world?" with exactly **one** color — enough for a plain backdrop, but it cannot paint a **sky**: no horizon, no zenith→horizon gradient, no distinct space backdrop. A planned future demo — where the player flies up out of a world's local bounds into open space — needs a convincing sky/space backdrop, not a flat fill. `Renderer::setSky(SkyParams)` (`include/renderer/Sky.h`) is that seam. The evaluation and design rationale (procedural gradient now; textured cubemap and a sun/star layer as recorded follow-ons) are in `docs/m17-skybox-evaluation.md`.

Like fog, the sky is a **policy, not a baked engine force**. The renderer owns only the *mechanism*: `SkyParams` carries a **zenith**, **horizon**, and **ground/nadir** color plus a horizon-band `falloff`, and the renderer paints a **camera-centered sphere** whose per-vertex color is `skyColor(dir, up)` — a vertical gradient between those three stops measured against the **camera up** vector (`setCameraUp`), so the horizon stays level on an arbitrary surface (the same gravity vector §18 threads through the camera basis and the shade ramp). Deliberately procedural, not a cubemap: it needs no texture asset pipeline and **no new shader** — the sky sphere is drawn through the *existing* `vs_voxel`/`fs_voxel` program (it samples the built-in 1×1 white tile, so the baked per-vertex color passes straight through), so it adds nothing to the committed shader-bytecode set. By choice of colors the same gradient is an atmospheric planet sky *or* a space backdrop, the two looks a space-flight demo would switch between.

**Correctness is by depth test alone.** The sphere is centered on the eye, so every forward view ray hits exactly one surface point (the far hemisphere is behind the near plane) — there is no self-overlap to cull. It is drawn at radius `0.999·farClip` with `DEPTH_TEST_LEQUAL` and **no depth write**: it passes on empty pixels (cleared depth 1.0) and is rejected wherever nearer geometry sits, whether the sky is submitted before or after that geometry, and regardless of backface culling. Fog is forced off for the sky submit (density 0) so the background is never self-fogged. The index buffer is built once; the per-vertex colors are recomputed into a transient buffer each frame from the current `SkyParams` + camera up.

The colors, band width, and any **animation** are supplied per frame by the game — typically the **procedural-sky** reference plugin (`plugins/procedural-sky`), a pure-policy supplier (fills a `psky::api()` table at init, registers no engine hooks — the fog-supplier shape) shipping `daySky()` / `duskSky()` / `spaceSky()` presets and an optional day/night cycle. **The default (`SkyParams::enabled == false`) draws no sky**, so every existing scene renders byte-identically — the flat clear color shows through exactly, and a renderer whose host never calls `setSky` paints no background sphere. `skyColor()` in `Sky.h` is the CPU mirror of the per-vertex gradient (used by tests and the supplier plugin).
...
FluidSystem / ThermalSystem (src/simulation/)
...
This section records the networking design. The subsystem is implemented and exercised by the `11-shared-world` demo and the `tests/Network*`/`EditReplication`/`JoinHandshake`/`InterestManagement`/`MessageEnvelope` suite; the decisions below describe the as-built behavior.
...
The following was added to `plugin_api.h` for networking. The existing `on_voxel_modified` hook is extended; all others are new:
...
This section records the audio design. The subsystem is implemented and exercised by the `12-soundscape` demo and the audio test suite; the decisions below describe the as-built behavior.
...
The reasoning is independent of the decision to freeze `MaterialProperties` (§5). Even with a thawed struct, sound would not belong on it:
...
The engine provides audio **primitives** — the sound registries plus the playback/emitter functions — and does **not** auto-play anything. Default behavior ships as a removable plugin, the audio sibling of the chat plugin (§15):
...
Consequently **audio adds no new event hook to `plugin_api.h`.** Audio rides the *existing* hooks (`on_voxel_modified`, the chunk-lifecycle hooks for decompose, and the structural/flow hooks). Because `on_voxel_modified` already carries a `source` field (§15), replicated remote edits produce local sound on every client — but no audio data ever crosses the wire (audio is out of networking's scope, §15); each client sounds its own copy of the replicated edit.

### Plugin API Surface for Audio

The following was added to `plugin_api.h` for audio. All are new; no existing hook changes.
...
POD/enum types added alongside: `AudioEvent` (`Footstep`, `Break`, `Place`; `Collapse`/`Flow` reserved for the structural/fluid events, §7/§17), `AttenuationModel`, and `SoundParams`/`EmitterParams` (volume, loop, attenuation model, min/max distance, rolloff, Doppler factor). No `std::` type crosses the plugin ABI, consistent with the rest of the surface.
...
Both channels are combined into a single overlay value per cell (max wins). The mesher (`buildChunkMeshData`) accepts an optional `LightQueryFn` callback and multiplies each face's vertex color by the sampled light level at the neighboring air voxel; when no callback is provided, the original fixed directional shading is byte-identical.
...
### Gravity Is a Policy, Not a Baked Force (L7)

Several early implementation choices quietly assumed a single privileged "down" axis (−Y): collision grounding hard-coded `axis == 1`, the fluid solver drained into `{x, y−1, z}`, and the textured-face / recipe-boundary code painted the "top" skin on +Y. That narrows the engine toward a Y-up block game even though the README promises flying, planetary, and space configurations. This section turns the implicit assumption into an explicit, configurable **policy**.
